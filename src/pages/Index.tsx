import { ArrowRight, Lightbulb, Mic, Music2, Users } from "lucide-react";
...
const features = [
  ...
  { num: "01", icon: Lightbulb, title: "Professional Quality", body: "We use industry-leading equipment and techniques to deliver world-class sound.", highlight: true },
  { num: "03", icon: Users, title: "Client-Focused Approach", body: "Your satisfaction is our priority — we don't stop until your sound is perfect." },
];
...
const packages = [
  ...
  { title: "Live Streaming Pro", items: ["2 hours of livestreaming", "LED screen", "Stream on Facebook & multiple platforms", "LED screen 13″ Sceen on Floor", "30 second BTS", "Vertical Pictures"] },
  ...
  return (
    <div className="app-shell">
      ...
      <section className="position-relative hero-pad overflow-hidden hero-section"> 
        ...
        <div className="container position-relative z-1 text-center" style={{ maxWidth: "65rem" }}>
          <p className="eyebrow-script">Signature Sound Studio</p>
          <h1 className="hero-title">Here Your Sound<br />Becomes a Signature.</h1>
          <p className="text-muted-custom mx-auto mb-4" style={{ maxWidth: "42rem" }}>
      ...
      <section className="">
        <div className="">
          ...
          {/* <img src={heroImg} alt="Recording studio interior" className="image-fill" width={1600} height={900} /> */}
        </div>
      </section>

      <section className="container page-section">
        <div className="row g-5 align-items-start justify-content-between mb-5">
          <div className="col-md-7">
            <h2 className="section-title mb-0">More Than a Studio —<br />Your Creative Media Partner</h2>
          </div>
          <div className="col-md-5 text-md-end">
            <p className="text-muted-custom mt-4 mb-0">From recording to final production, we work with you every step of the way to deliver sound that stands out.</p>
          </div>
        </div>
        <div className="d-flex align-items-start flex-md-row">
          {features.map((feature) => {
            const Icon = feature.icon;
            return (
              <div className="col-md-4" key={feature.title}>
                <article className={`feature-card ${feature.highlight ? "feature-card-highlight" : ""}`}>
                  <span className="feature-icon"><Icon className="icon-md" /></span>
                  <p className={feature.highlight ? "mb-2 opacity-75" : "text-muted-custom mb-2"}>{feature.num}.</p>
                  <h3 className="font-display mb-3">{feature.title}</h3>
                  <p className={feature.highlight ? "mb-0 opacity-75" : "text-muted-custom mb-0"}>{feature.body}</p>
                </article>
              </div>
            );
          })}
      ...
      <section className="containStudio">
        <h2 className="section-title mb-5">Our Studio Spaces</h2>
        <div className="studio-parent">
          {[{ img: musicImg, label: "Music Studio" }, { img: podcastImg, label: "Podcast Studio" }, { img: musicImg, label: "Music Studio" }, { img: musicImg, label: "Music Studio" }].map((space) => (
            <div className="studioCol" key={space.label}>
              <div className="media-card">
                <img src={space.img} alt={space.label} loading="lazy" width={1024} height={768} />
              </div>
      ...
      <section className="container page-section">
        ...
        <div className="row g-4">
          {packages.map((pack) => (
            <div className="col-sm-6 col-lg-4" key={pack.title}>
              <article className="package-card h-100">
                <h3 className="font-display fs-4 mb-3">{pack.title}</h3>
                <ul className="dash-list">
                  {pack.items.map((item) => <li key={item}><span>{item}</span></li>)}
                </ul>
              </article>
            </div>
          ))}
      ...
      <section className="page-section">
        <div className="d-flex g-5 align-items-start justify-content-between flex-md-row flex-column">
          <div className="action-section">
            <h2 className="action-hero-title mb-4">Come create<br /><span className="text-gold-gradient">magic</span> with us</h2>
          ...
          <div className="">
            <div className="mediaCard aspect-square">
              <img src={magicImg} alt="Vintage microphone" loading="lazy" width={1024} height={1024} />
            </div>
          </div>
        </div>
      </section>
